...
The current IP extractor follows the local 2025 CakeCarousel server's approach: take the first x-forwarded-for value and strip the ::ffff: IPv4 prefix. Validated Vercel-specific headers remain fallbacks; untrusted local requests do not gain header trust. The poster client reports tracking HTTP/network failures in the browser console without disrupting playback. A deployed /health response containing only status=ok is older than the versioned IP-handler changes and must be redeployed from the updated server source.
